...
@WebServlet({"/user/login","/user/exit","/user/reg","/user/for"})
public class Userservlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        }else  if("/user/for".equals(servletPath)){
            doForget(request,response);
        }
    }

    private void doForget(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException{
        ...
        try {
            ...
            if (username.length()==0 || email.length()==0) {
                JOptionPane.showMessageDialog(null,"用户名或邮箱不得为空，请重试！","消息",JOptionPane.WARNING_MESSAGE);
                response.sendRedirect("/oa/forget.jsp");
            }else {
                if (rs.next()) {
                    ...
                    JOptionPane.showMessageDialog(null,"用户名或邮箱错误，请重试！","消息",JOptionPane.WARNING_MESSAGE);
                    response.sendRedirect("/oa/forget.jsp");
                }
            }
